...
public class ItemServiceImp implements ItemService {
    ...
    public ItemDto updateItem(ItemDto itemDto, Integer itemId, Integer userId) {
        ...
        User user = userRepository.findById(userId).orElseThrow(() -> new NotFoundException("User not found"));
        ItemRequest request = requestRepository.findById(itemDto.getRequestId()).orElseThrow(() -> new NotFoundException("Request not found"));
    ...
    public ItemCommentDto getItem(Integer itemId, Integer userId) {
        ...
        if (itemBookings.size() > 1) {
            ...
            if (!pastBookings.isEmpty()) {
                lastBooking = bookingMapper.toBookingDto(pastBookings.getLast());
            } else {
                lastBooking = null;
            }
            if (!futureBookings.isEmpty()) {
                nextBooking = bookingMapper.toBookingDto(futureBookings.getFirst());
            } else {
                nextBooking = null;
            }
        } else {
